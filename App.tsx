...
const App: React.FC = () => {
  ...
  const performGeneration = useCallback(async () => {
    ...
    try {
      ...
      for (const variation of variationLabels) {
        ...
        // 2. Generate Image
        // Use Pro API key if available and Pro model is selected, otherwise use standard key
        const activeApiKey =
          selectedModel === "pro" && userProApiKey ? userProApiKey : userApiKey;
  ...
  const currentAngleLabel =
    ANGLES.find((a) => a.id === selectedAngle)?.label || "指定なし";

  // Group items for Collection Room
  const squareItems = gallery.filter((i) =>
    [AspectRatio.Square, AspectRatio.Grid3x3].includes(i.aspectRatio)
  );
  const portraitItems = gallery.filter((i) =>
    [AspectRatio.Portrait, AspectRatio.Tall, AspectRatio.Kindle].includes(
      i.aspectRatio
    )
  );
  const landscapeItems = gallery.filter((i) =>
    [AspectRatio.Landscape, AspectRatio.Wide, AspectRatio.XHeader].includes(
      i.aspectRatio
    )
  );
    ...
    <div className="min-h-screen pb-12" ref={topRef}>
      ...
      <header className="sticky top-0 z-40 bg-black border-b-4 border-yellow-600 shadow-md w-full">
        <div className="max-w-lg mx-auto px-2 sm:px-4 py-3 flex items-center justify-between">
          ...
          >
            <div
              key={bananaAnimationKey}
              className={`relative ${bananaAnimation}`}
            >
              <BananaIcon size={28} className="sm:w-8 sm:h-8" />
            </div>
            <div>
              <h1
                className="text-lg sm:text-xl font-bold text-white tracking-widest leading-none"
                style={{
                  textShadow:
              ...
              <p className="text-[9px] sm:text-[10px] text-yellow-400 font-mono tracking-widest">
                MAGIC FACTORY
              </p>
            </div>
          </div>
      ...
      <div className="bg-[#06C755] border-b-4 border-black text-center py-2 px-4 shadow-sm relative overflow-hidden group animate-flash-occasionally">
        ...
        >
          ...
          最新の装備(ツール)配布はこちら！
          <span className="bg-white text-[#06C755] px-1 rounded text-[10px] font-black">
            LINE
          </span>
          <span className="animate-pulse">✨</span>
        </a>
      ...
      <main className="max-w-md mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-yellow-100 border-4 border-yellow-600 p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)] flex gap-3 items-start">
          ...
          <div className="text-xs text-yellow-900 font-bold space-y-1">
            <p className="text-sm underline decoration-yellow-700 decoration-2 underline-offset-2 mb-1">
              {t.shop_warning}
            </p>
            <p>{t.gold_warning}</p>
        ...
        {(isGenerating || isLooping) && (
          <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div
              className="mb-2 text-white text-2xl font-bold animate-pulse text-center"
            ...
            <div className="mb-6 text-yellow-300 text-sm font-bold text-center bg-black/50 px-4 py-2 rounded-lg border-2 border-yellow-500 shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
              {language === "ja"
                ? "魔法を詠唱中... 暇つぶしにどうぞ！"
                : "Casting spells... Enjoy this game while waiting!"}
            </div>
        ...
        <div className="space-y-1">
          {ANGLES.map((angle) => (
            <button
              ...
              className={`w-full text-left p-3 border-b-2 border-dashed border-gray-300 hover:bg-yellow-50 transition-colors flex items-center justify-between group ${
                selectedAngle === angle.id ? "bg-yellow-100" : ""
              }`}
            >
              <div>
                <div className="font-bold text-sm group-hover:text-yellow-700">
                  {angle.label}
                </div>
      ...
      <Modal isOpen={isHelpOpen} onClose={toggleHelp} title={t.help_title}>
        <div className="space-y-6 text-sm font-medium leading-relaxed">
          <div className="bg-yellow-50 p-4 border-l-4 border-yellow-400">
            <p className="mb-2">
              <strong>Nanobanana Magicへようこそ！</strong>
              <br />
          ...
          <div>
            <h4 className="border-b-2 border-black inline-block mb-3 font-bold bg-yellow-200 px-1">
              クラフトの手引き
            </h4>
            <ul className="list-disc pl-5 space-y-2 text-gray-700">
              <li>
                <strong>呪文と世界観:</strong>{" "}
                作りたいもののキーワードを入力し、好みのテーマを選んでください。
              </li>
              <li>
                <strong>素材装備:</strong>{" "}
                画像をアップロードすると、そのキャラクターや構図を反映させることができます。
              </li>
              <li>
                <strong>アングル指定:</strong>{" "}
                カメラの視点を変更して、迫力ある構図を狙えます。
              </li>
              <li>
                <strong>REMIX (リレー):</strong>{" "}
                他の画像の「REMIX」ボタンを押すと、その呪文を引き継いで続きを作れます。
              </li>
              <li>
                <strong>CREATE ITEM:</strong> 1つだけ生成します。
              </li>
              <li>
                <strong>AUTO CRAFT:</strong>{" "}
                停止ボタンを押すか、最大20回に達するまで自動で生成し続けます。
              </li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="list-disc pl-5 space-y-2 text-gray-700">
              <li>
                <strong>STD (Flash) - 無料:</strong>
                <br />
                錬成速度が速い標準モデル。一般的なアイテム生成に最適です。無料で使えます。
              </li>
              <li>
                <strong>LEGEND (Pro) - ゴールド必須:</strong>
                <br />
          ...
          <div className="text-xs text-gray-400 border-t border-gray-200 pt-4">
            <p>
              Security Note: API Key is stored only in your browser session.
              この工房は安全です。
            </p>
          </div>
        </div>
